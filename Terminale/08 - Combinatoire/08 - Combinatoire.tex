...
\DeclareExerciseCollection[topic=comb01]{comb01}
\DeclareExerciseCollection[topic=comb02]{comb02}
\DeclareExerciseCollection[topic=comb03]{comb03}
\DeclareExerciseCollection[topic=comb04]{comb04}
...
\begin{exercise}[topic=comb03]Dans une grille de loto, il faut choisir cinq nombres de 1 à 49 ainsi qu'un nombre chance allant de 1 à 10. De combien de manières différentes peut-on remplir sa grille de loto ?\end{exercise}

\begin{solution}On a donc $\dbinom{49}{5} \times \dbinom{10}{1} = 19068840$ manières de remplir une grille de loto.\end{solution}



\begin{exercise}[topic=comb03]On considère un jeu de 32 cartes. Chaque carte possède une couleur (Cœur, Pique, trèfle, Carreau) et une valeur (As, Roi, Dame, Valet, 10, 9, 8, 7). Une main est un ensemble de 5 cartes tirées dans ce paquet, sans tenir compte de l'ordre des cartes tirées. Déterminer le nombre de mains :
\begin{itemize}
...
\printcollection{comb04}

\chapter{Corrigés}

\section*{Cardinal d'ensembles}
\printsolutions[collection={comb01}, headings={false} ]


\section*{Arrangements et permutations}

\printsolutions[collection={comb02}, headings={false} ]


\section*{Combinaisons d'un ensemble fini}

\printsolutions[collection={comb03}, headings={false} ]

\section*{Exercices de synthèse}
\printsolutions[collection={comb04}, headings={false} ]
